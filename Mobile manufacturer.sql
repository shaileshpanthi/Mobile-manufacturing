...
SELECT DISTINCT(Location.STATE) FROM 
	DIM_LOCATION AS Location
	LEFT JOIN FACT_TRANSACTIONS AS Transactions ON Location.IdLocation = Transactions.IdLocation
	WHERE YEAR(Transactions.DATE) >= 2005

------Q2. What state in the US is buying more Samsung cell phones?

SELECT distinct (Location.STATE) FROM DIM_MODEL AS Model 
	LEFT JOIN FACT_TRANSACTIONS AS Transact ON Model.IdModel = Transact.IdModel
	LEFT JOIN DIM_LOCATION AS Location ON Transact.IdLocation = Location.IdLocation 
	LEFT JOIN DIM_MANUFACTURER AS Maker ON Model.IDManufacturer = Maker.IDManufacturer
	WHERE Maker.Manufacturer_Name = 'SAMSUNG' AND Location.Country = 'US'

------Q3. Show number of transaction for each model per Zip code per state.

SELECT COUNT(*) AS NO_OF_TRANSACTIONS 
	FROM (SELECT distinct Transactions.IDMODEL, Location.ZIPCODE, 
			Location.STATE FROM FACT_TRANSACTIONS AS Transactions
			LEFT JOIN DIM_LOCATION AS Location ON Transactions.IdLocation = Location.IdLocation) AS NT
...
SELECT Model.MODEL_NAME, round(AVG(Transactions.TOTALPRICE),2) AS AVERAGE_PRICE FROM DIM_MODEL AS Model 
	
	LEFT JOIN FACT_TRANSACTIONS AS Transactions ON Model.IDModel = Transactions.IDModel
	LEFT JOIN DIM_MANUFACTURER AS Maker ON Model.IDManufacturer = Maker.IDManufacturer 
	
	WHERE Maker.Manufacturer_Name 
	IN  (
		SELECT TOP 5 (Maker.Manufacturer_Name) 
		FROM DIM_MODEL AS Model LEFT JOIN FACT_TRANSACTIONS AS Transactions
		ON Model.IDModel = Transactions.IDModel
		LEFT JOIN DIM_MANUFACTURER AS Maker
		ON Model.IDManufacturer = Maker.IDManufacturer 
		GROUP BY Maker.Manufacturer_Name
		ORDER BY SUM(Transactions.Quantity) desc
		)

GROUP BY Model.Model_Name
ORDER BY AVG(Transactions.TOTALPRICE) desc

-------Q6.List the names of customers and average amount spend in 2009, where the average is higher than 500. 
 
SELECT Customer.CUSTOMER_NAME, AVG(Transactions.TotalPrice) 
	FROM DIM_CUSTOMER AS Customer INNER JOIN FACT_TRANSACTIONS AS Transactions 
	ON Customer.IdCustomer = Transactions.IdCustomer
	WHERE YEAR(Transactions.DATE) = 2009
	
	GROUP BY Customer.Customer_Name
	HAVING AVG(Transactions.TOTALPRICE) > 500
...
(
	SELECT * FROM 
		(
			SELECT TOP 5 Model.Model_Name FROM DIM_MODEL AS Model LEFT JOIN FACT_TRANSACTIONS AS T ON Model.IdModel = T.IdModel 
			WHERE YEAR(DATE) = 2008
			GROUP BY Model.Model_Name
			ORDER BY SUM(T.Quantity) DESC)X
			UNION ALL
				SELECT * FROM 
					(
						SELECT TOP 5 Model.Model_Name  FROM DIM_MODEL Model LEFT JOIN FACT_TRANSACTIONS T ON Model.IdModel = T.IdModel 
			...
			SELECT * FROM 
				(
					SELECT TOP 5 Model.Model_Name   FROM DIM_MODEL AS Model LEFT JOIN FACT_TRANSACTIONS T ON Model.IdModel = T.IdModel 
...
SELECT DISTINCT(YEAR(T.DATE)) AS YEAR, Maker.MANUFACTURER_NAME
	
	FROM DIM_MANUFACTURER AS Maker LEFT JOIN DIM_MODEL AS Model ON Maker.IdManufacturer = Model.IdManufacturer
	 LEFT JOIN FACT_TRANSACTIONS T ON Model.IdModel = T.IdModel
	WHERE(YEAR(T.DATE) = 2009 )AND T.TotalPrice =  (SELECT MAX(TotalPrice) 
     ...
     OR
		(YEAR(T.DATE) = 2010)AND T.TotalPrice =  (SELECT MAX(TotalPrice) 
...
-----Q9. Show the manufacturers sold cellphone in 2010 but didn't in 2009.

SELECT DISTINCT(Maker.MANUFACTURER_NAME) FROM DIM_MANUFACTURER AS Maker 
	INNER JOIN DIM_MODEL AS Model ON Maker.IdManufacturer = Model.IdManufacturer 
	INNER JOIN FACT_TRANSACTIONS AS T ON Model.IdModel = T.IdModel
	
WHERE YEAR(T.DATE) = 2010 AND YEAR(T.DATE) != 2009

----Q10.Find top 100 customers and their average spend, avg quantity by each year. Also find the percentage in their spend.

SELECT TOP 100 C.CUSTOMER_NAME,
	AVG(CASE WHEN YEAR(T.DATE) = 2003 THEN (T.TOTALPRICE) END) AS SPENT_2003 ,
	AVG(CASE WHEN YEAR(T.DATE) = 2003 THEN (T.QUANTITY) END) AS QUANTITY_2003 ,
	AVG(CASE WHEN YEAR(T.DATE) = 2004 THEN (T.TOTALPRICE) END) AS SPENT_2004 ,
	AVG(CASE WHEN YEAR(T.DATE) = 2004 THEN (T.QUANTITY) END) AS QUANTITY_2004 ,
	AVG(CASE WHEN YEAR(T.DATE) = 2005 THEN (T.TOTALPRICE) END) AS SPENT_2005 ,
	AVG(CASE WHEN YEAR(T.DATE) = 2005 THEN (T.QUANTITY) END) AS QUANTITY_2005 ,
	AVG(CASE WHEN YEAR(T.DATE) = 2006 THEN (T.TOTALPRICE) END) AS SPENT_2006 ,
	AVG(CASE WHEN YEAR(T.DATE) = 2016 THEN (T.QUANTITY) END) AS QUANTITY_2006 ,
	AVG(CASE WHEN YEAR(T.DATE) = 2007 THEN (T.TOTALPRICE) END) AS SPENT_2007 ,
	AVG(CASE WHEN YEAR(T.DATE) = 2007 THEN (T.QUANTITY) END) AS QUANTITY_2007 ,
	AVG(CASE WHEN YEAR(T.DATE) = 2008 THEN (T.TOTALPRICE) END) AS SPENT_2008 ,
	AVG(CASE WHEN YEAR(T.DATE) = 2008 THEN (T.QUANTITY) END) AS QUANTITY_2008 ,
	AVG(CASE WHEN YEAR(T.DATE) = 2009 THEN (T.TOTALPRICE) END) AS SPENT_2009 ,
	AVG(CASE WHEN YEAR(T.DATE) = 2009 THEN (T.QUANTITY) END) AS QUANTITY_2009 ,
	AVG(CASE WHEN YEAR(T.DATE) = 2010 THEN (T.TOTALPRICE) END) AS SPENT_2010 ,
	AVG(CASE WHEN YEAR(T.DATE) = 2010 THEN (T.QUANTITY) END) AS QUANTITY_2010											

FROM DIM_CUSTOMER AS C LEFT JOIN FACT_TRANSACTIONS AS T ON C.IdCustomer = T.IdCustomer
		
GROUP BY  C.CUSTOMER_NAME
ORDER BY SUM(T.TOTALPRICE)
